...
//        self.makeToast(message, duration: duration, position: position, style: nil)
    ...
    */
    ...
    private func showToast(toast: UIView, duration: TimeInterval, position: CGPoint) {
        ...
        if ToastManager.shared.tapToDismissEnabled {
            let recognizer = UITapGestureRecognizer(target: self, action: #selector(self.handleToastTapped)) /// check here
        ...
        UIView.animate(withDuration: ToastManager.shared.style.fadeDuration, delay: 0.0, options: [.curveEaseOut, .allowUserInteraction], animations: { () -> Void in
            ...
            let timer = Timer(timeInterval: duration, target: self, selector: #selector(self.toastTimerDidFinish), userInfo: toast, repeats: false)
    ...
    */
    ...
    private func centerPointForPosition(position: ToastPosition, toast: UIView) -> CGPoint {
        ...
        case .Bottom:
            return CGPoint(x: self.bounds.size.width / 2.0, y: (self.bounds.size.height - (toast.frame.size.height / 2.0)) - padding-100)
        }
    }
}
...
struct ToastStyle {
   ...
   // var backgroundColor = UIColor.blackColor().colorWithAlphaComponent(0.8)
     //var backgroundColor = UIColor(red: 51.0/255.0, green: 204.0/255.0, blue: 255.0/255.0, alpha: 0.8)
    
     var backgroundColor = UIColor(red: 0.0/255.0, green: 0.0/255.0, blue: 0.0/255.0, alpha: 0.5)
